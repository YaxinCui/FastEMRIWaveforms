...
<!-- 2026-09-04 11:41 CST (linux): Index the design-first FEW multilayer
problem audit and distinguish evidence-ranked findings from the earlier
speculative next-generation blueprint. -->

<!-- 2026-09-04 12:05 CST (linux): Index the second cross-disciplinary
problem-discovery report, the expanded 55-PDF archive, and the living web/code
source index without changing production implementation claims. -->

<!-- 2026-09-04 13:37 CST (linux): Index the third adversarial failure audit
and the 65-PDF archive after examining convention, validity, validation,
call-history, concurrency, population, and runtime-integrity failure modes. -->

<!-- 2026-09-04 18:43 CST (linux): Restore the text guides, manifests, and
Apple reports from the deep-optimization stash onto the current codex CUDA
branch, and index the phase-four update backed by the new 5x measurements. The
local PDF payloads were already present and were not rewritten. -->
...
The current design-level problem map is
[FEW_MULTILAYER_PROBLEM_AUDIT.md](FEW_MULTILAYER_PROBLEM_AUDIT.md). It separates
model scope, confirmed defects, evidence gaps, and research hypotheses across
scientific use, physics, approximation, waveform representation, offline data,
numerics, detector/statistics, algorithms, hardware, software, validation, and
reproducibility. Use it as the governing problem statement before selecting an
optimization experiment.

The second-pass synthesis is
[FEW_PHASE2_CROSS_DISCIPLINARY_HYPOTHESES.md](FEW_PHASE2_CROSS_DISCIPLINARY_HYPOTHESES.md).
It reorganizes the project around three cross-layer fault lines—scientific
semantics, parameter-space geometry, and workload—and records 74 evidence-ranked,
falsifiable questions plus ten route-changing experiments. Its findings are a
hypothesis backlog, not an implementation-completion claim.

The third-pass adversarial audit is
[FEW_PHASE3_ADVERSARIAL_FAILURE_AUDIT.md](FEW_PHASE3_ADVERSARIAL_FAILURE_AUDIT.md).
It asks how FEW can return smooth, finite, cross-backend-consistent results that
are still scientifically wrong. It adds 68 falsifiable problems and 12
experiments covering mass/frame conventions, parameter equivalence classes,
validity firewalls, circular validation, population bias, event gradients,
history-dependent likelihoods, concurrency, and runtime integrity.

The current-branch empirical correction is
[FEW_PHASE4_EMPIRICAL_EVIDENCE_UPDATE.md](FEW_PHASE4_EMPIRICAL_EVIDENCE_UPDATE.md).
It upgrades or downgrades earlier hypotheses using the master-rooted RTX 2080 Ti
implementation, the complete 5.09 GB Kerr table, synchronized stage profiling,
seven physical regimes, and CPU/CUDA regressions. It records Q143--Q166 and
keeps the achieved 5.82x long-waveform speed target separate from the still-
missing detector-weighted scientific acceptance.
...
[library/MANIFEST.tsv](library/MANIFEST.tsv); the 65 PDF payloads totaling
171,399,038 bytes are ignored by Git and independently reconstructible on
either host. Living official documentation and maintained code are separated
into [library/PHASE2_WEB_INDEX.md](library/PHASE2_WEB_INDEX.md).
...
To guide subsequent algorithm optimization and GPU mixed-precision acceleration, three deep domain guides, an authenticated 65-paper local PDF archive, and curated open-source reference repositories have been integrated into the local workspace:
...
5. **[Authenticated Literature Archive (65 Local PDFs)](library/INDEX.md)**:
   - All 65 primary literature papers/reports (171,399,038 bytes) are locally downloaded and tracked with exact SHA-256 digests in [`library/MANIFEST.tsv`](library/MANIFEST.tsv) under the Git-ignored `library/downloads/`.
