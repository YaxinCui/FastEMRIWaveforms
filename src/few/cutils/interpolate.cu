...
// Solve a single tridiagonal system A x = rhs by Gaussian elimination with
// partial pivoting.
//
//   n   : system size
//   dl  : sub-diagonal,   dl[i]  = A(i+1, i), i <= n-2
//   d   : diagonal,       d[i]   = A(i, i),   i <= n-1
//   du  : super-diagonal, du[i]  = A(i, i+1), i <= n-2
//   rhs : right-hand side
//   du2 : scratch space of at least n-2 elements, holding the second
//         super-diagonal fill-in generated by row interchanges
//
// dl, d and du and rhs are overwritten in-place.
int tridiagonal_solve(int n, double *dl, double *d, double *du, double *rhs, double *du2)
{
  if (n <= 0)
    return 0;
  ...
  {
    if (std::fabs(d[i]) >= std::fabs(dl[i]))
    {
      // pivot is already the largest entry: no row interchange needed
      if (d[i] == 0.0)
        return i + 1;
    ...
    {
      // interchange rows i and i+1, which fills in the second super-diagonal
...
// solve each system in turn on CPU

  // scratch for the second super-diagonal fill-in, reused across systems
  std::vector<double> du2(m > 2 ? m - 2 : 0);

  for (int j = 0;
       j < n;
       j += 1)
  {
    tridiagonal_solve(m, &a[j * m + 1], &b[j * m], &c[j * m], &d_in[j * m], du2.data());
  }
