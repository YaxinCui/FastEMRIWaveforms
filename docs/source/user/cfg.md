...
- `metal`: Explicit Apple Silicon hybrid using NumPy storage and the Metal GPU
  for strict time-domain mode summation

<!-- 2026-09-02 13:55 CST (mac): Document Metal as explicit-only and outside
the automatic CUDA/CPU module preference lists. -->
...
The `metal` backend is outside automatic module preference lists and must be
requested with `force_backend="metal"`.
...
- On an Apple Silicon Mac, `enabled_backends = ['cpu', 'metal']` makes the
  hybrid available to code that explicitly passes `force_backend="metal"`.
...
| `enabled_backends` | `enabled-backends` | `FEW_ENABLED_BACKENDS` | `--enable-backend` | `cpu`, `cuda12x`, `metal` | ";"-separated list of values. The CLI parameter can be used multiple time for each enabled backend. |
